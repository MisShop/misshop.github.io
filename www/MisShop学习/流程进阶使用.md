...
2、	配置流程（假定报销审批所需流程为：提交、部门审批、财务审批、出纳审批 财务执行 审批结束，其中出纳审批和财务审批为会签模式<俩个部门均同意后方可进行财务执行阶段，无先后之分>）
点击上图中的配置按钮进入流程图配置  
